# T7: One Management Only

**Expected result: FAIL**

Only `sidj-ubq` (Management) approves. `siddharthjaincodes` is placed in `qvac-internal-dev` for write access but does not approve. codeowners=1 but total=1 — fails the `total >= 2` condition.

## Approvers

| Account | Team | Role | Approves? |
|---------|------|------|-----------|
| sidj-ubq | qvac-mgmt | Management | Yes |
